...
本文面向初学者，每一步都比较详细。阅读本文能学习一些**命令行、程序编译和调试**，以及使用VS Code的知识。如果嫌本文说的麻烦，我给个精简版的：装gcc和c/c++扩展，打开文件夹，点开源代码，F1，build and debug active file，完。

本文许多内容都可从VS Code官方文档：[C++ programming with Visual Studio Code](https://link.zhihu.com/?target=https%3A//code.visualstudio.com/docs/languages/cpp) 以及各个扩展的文档中获得，并且他们还会进行更新；本文也进行过几次重大更新。
...
“安装”编译器：下下来的是一个7z的压缩包。如果不会解压可以百度“压缩包怎么解压”。解压完了放到一个不容易被删的地方，层叠的可以去掉一些。看好bin文件夹的完整路径，我图里的是C:\mingw64\bin，把它加到环境变量中的PATH里去。如果不会可以百度怎么添加环境变量，或者看本文最后面的“B. 如何添加环境变量”（可以在本页用Ctrl+F搜索）。

Debian系Linux用`sudo apt update && sudo apt install build-essential`即可。

![img](https://pic3.zhimg.com/50/v2-5a148b7365261adeeb10da8b2f5e10e2_720w.jpg?source=1940ef5c)

![img](https://pic3.zhimg.com/v2-ccf8dc5342d1f81e74dbd6018e36e61b_r.jpg?source=1940ef5c)顺序不重要；路径可以不一样，保证gcc.exe在那个文件夹里就行
...
这两项验证**一定要符合**，否则必须修改环境变量。小心**别错删**。

![img](https://pic1.zhimg.com/v2-c8863ed68567601fd7f7b69adc41f174_r.jpg?source=1940ef5c)

![img](https://pic1.zhimg.com/v2-b63d57e63e10a991441b23ec45be86f6_r.jpg?source=1940ef5c)输入gcc -v的最后一行输出，版本要和你自己下的对应
...
创建一个你打算存放代码的文件夹，称作工作区文件夹；**路径不能含有中文和引号**，最好不要有空格，我用的是`C:\VS-Code-C`。C和C++需要分别建立不同的文件夹。不要选上一节存放编译器的文件夹，源代码和编译器要分开放。

打开VSC，**选打开文件夹**；最好不要选“添加工作区文件夹”，这个就是虚拟工作区，我没用过，不保证没问题。点新建文件夹，名称为`.vscode`；如果用的是Win10 1903+也可以在资源管理器里新建。然后创建 launch.json，tasks.json，settings.json（**不是setting.json**） 放到**.vscode文件夹下**：

![img](https://pic1.zhimg.com/50/v2-2dd6bdc8a62d70b48747fb92ffe71c57_720w.jpg?source=1940ef5c)一定要在.vscode里，别变成平行的了

这几个文件的内容见下。复制以下代码出来后，知乎会自动在前面加上几行保留所有权利的字，实际使用的时候肯定要删了的。有些地方可选修改，自己对照着注释看吧。注意：如果是写C++，tasks.json的一个地方必须要修改。

### launch.json代码

*externalConsole*可根据自己喜好修改；cwd可以是程序运行时的相对路径，如有需要可以改为*${fileDirname}*（感谢 [@xhx](https://www.zhihu.com/people/dd30faca74afd3e29376d2226d44475d)）；type和request不变色是正常现象。
...
本来2018年10月后把externalConsole设为false可以使用内置终端，但2019年10月cpptools 0.26.1引入了一个bug，导致Win下用内置终端无法输入中文，到现在0.29.0仍没有解决。我已经open了 [https://github.com/microsoft/MIEngine/pull/1025](https://link.zhihu.com/?target=https%3A//github.com/microsoft/MIEngine/pull/1025) 来fix，但是过了几个月他们还没有review，差评。

### tasks.json代码

如果是**编写C++**，编译器需改成g++。MinGW的-std=c++17好像有问题，最好至多到c++14，但好像默认用的就是合适的，c语言版本也超过了c11，所以就别加-std了。Linux下不需要加-fexec-charset。反正这些我都加了注释，还看不懂，百度gcc使用教程。
...
新建文件，c语言源代码后缀是.c，c++是.cpp。代码文件在保存工作区内都可以，可以自己建立文件夹，**不必放到.vscode文件夹里**，但**路径里(包括文件名)不要含有中文和引号**，最好不要有空格。主要是许多符号是有效的shell语法，不然试试Linux下用rm删除一个叫做-rf的文件？没查过绝对写不出来。

按Alt+Shift+F（或者用右键菜单）可以格式化代码。出现Intellisense的时候**按tab可以补全代码**。打出snippets时会出现多个跳转点，按tab可以跳到下一个去。

停止输入大概一秒后会有Lint，能实时检测出一些错误，有时还会给一些建议性的warning，比如声明了变量但不使用。如果觉得不爽，也有方法不让它提示，比如去掉-Wall就会少一些，不过错误还是越早发现越好。另外cpptools的Lint不支持设定Flags，导致用处大大减弱，差评，Follow：[Error and Warning Flags? · Issue #2814 · microsoft/vscode-cpptools](https://link.zhihu.com/?target=https%3A//github.com/microsoft/vscode-cpptools/issues/2814)

按Ctrl+Shift+B单纯编译，按F5为编译加调试。写大作业时，建议时不时就用一下Ctrl Shift B，能尽早发现错误。本来ctrl+F5为运行但不调试，但cpptools暂不支持，Follow: [Support "Run without debugging" · Issue #1201 · microsoft/vscode-cpptools](https://link.zhihu.com/?target=https%3A//github.com/Microsoft/vscode-cpptools/issues/1201)

加断点在列号前面点一下，右键可以加条件断点。加在main函数就在一开始就停下来。开始调试后，按f11可以一步一步进行，箭头所指的那行代码就是**下一步要运行的代码**；f5是一直运行到下一个断点，右键某一行代码可以选择一直运行到指定的那一行。对于正常的语言，这样就可以了，但cpptools不支持just my code，导致f11会进到库函数比如printf中，超级差评。只能平常用f10，遇到调用自己的函数时再用f11。Follow：[Add support for "Just My Code" debugging · Issue #5763 · microsoft/vscode-cpptools](https://link.zhihu.com/?target=https%3A//github.com/microsoft/vscode-cpptools/issues/5763)
...
特别的，对于数组：C语言的数组经过函数传递以后会退化为指针，直接添加表达式只能看到第一个元素。此时可以强制转换成指向固定大小的数组指针再解引：例如`int arr[10]`传进函数里后就变成了`int* arr`，在Watch里添加`*(int(*)[10])arr`，这样就能看到完整的数组了。但长度必须是写死的，自己小心越界。或者简单的程序用全局变量数组就能一直看到了。另一种只对gdb且是非void*有效的写法：`*arr@10`。
...
在终端中按Ctrl + C可以终止程序运行，下一次运行前必须保证当前程序已经终止了（对于task也是一样的）。如果你想要复制，选中内容后直接按一下右键就可以了；粘贴则是在未选中时按右键；这个操作仅限Win10。ctrl+c也可以复制但可能一不小心就把程序终止了，Linux下则是ctrl shift c复制。

用它还可以在非工作区内编译运行程序，不过就最好把相关设置放到全局设置里。按照我的配置，task和Code Runner还有一点不同：working directory，前者是你打开的文件夹，后者是文件所在的文件夹。当然它们也都可以自己修改。
...
以后**写代码必须打开之前那个建立好的文件夹**才能写，否则所有的Intellisense都没有，只有Code Runner能用。主要就是需要那几个json，新建其它文件夹后把那几个json复制过去就也能用，当然也可以用软连接。

可以创建一个快捷方式（右键新建），把工作区路径作为参数传给VSC主程序，记得打双引号；还可以加个图标。1.18有了真正的虚拟工作区，可以一个窗口包含多个不在一起的文件夹，“文件”菜单里也有“保存工作区”这个功能，但是我都没试过，不保证没问题。

![img](https://pic2.zhimg.com/v2-ef8da0bfd7f5e03cfd9ed0354c7c19ec_r.jpg?source=1940ef5c)
...
首先如果完全按照本文的配置，不会有乱码问题。Linux也没有问题。

VS Code输出中文很多人都遇到过乱码，比如把“你好”输出成“浣犲ソ”。这是因为VSC的源代码默认是UTF-8编码，cmd/PowerShell是GBK编码。

因为历史原因，编码的坑很多。作为初学者只要知道UTF8最好就行了，所以本文没有修改VSC的默认文件编码。但是Windows的shell又必须用GBK，因为改成UTF8时没法输入，本文的解决办法是使用gcc的-fexec-charset=GBK，即编译后的运行环境是GBK。

使用VSC打开其它同学用Dev Cpp或CB编写的代码文件时需要手动选择用GBK打开，或者打开编码猜测，否则文件内容就会乱码。

C语言的宽字符wchar.h涉及了此部分，但我强烈不建议学。
...
- [VS Code 中文注释显示乱码怎么办？](https://www.zhihu.com/question/34415763/answer/596058039)

## 6. 找不到头文件的错误

![img](https://pic3.zhimg.com/50/v2-877058be4208e6b3648a1da6cec6205e_720w.jpg?source=1940ef5c)
...
![img](https://pic2.zhimg.com/v2-a7e1578ac1d50bcba9ccc3986ae607ad_r.jpg?source=1940ef5c)

这种情况是因为clang的默认target为msvc，需要加`--target=x86_64-w64-mingw`这个参数才行。这个默认target是写死在源代码里的，我找了一圈没找到正常修改办法。下载clang的源代码，自己改掉，再编译clang本身，也许可以解决。或者装Windows SDK而不使用mingw，这样就符合默认target了。
...
- [mstorsjo/llvm-mingw](https://link.zhihu.com/?target=https%3A//github.com/mstorsjo/llvm-mingw)：基于MinGW-W64的Clang，target默认不为msvc，但预构建的包没有clangd
...
- clang的execution-charset supports only UTF-8
...
体积上，本体+编译器+扩展，如果只是用来写C，硬盘占用并不算小，上1G了。内存占用还是比较少的，0.5g左右。

VSC的第一优势也许是好看？虽然不是专门为C/C++设计的，但它应该是现在最现代化的纯文本编辑器了。而且光Lint这一点就比wintc、cfree、dev c++强了很多了，更别提dev c++的Debug功能自己就有bug。

其它IDE，CodeBlocks还活着，但历史包袱非常明显。Clion界面美观，功能也挺强，不过只有英文，刚上手用起来可能有点困难；学生可以免费申请key，否则收费。

## 9. 感想

本文最大的特点，就是我在最开头写的：能学习一些命令行、程序编译和调试，以及使用VS Code的知识。**本文很长，但实际操作难度并不大；引入的诸多名词，目的是让初学者快速建立起编程的概念，而且是学校里的老师不会教的概念，是会永远影响之后编程学习的基础。**

总之，能搜到这篇文章，努力去看懂、动手配置，已经比贴吧无数伸手党和等着老师在qq群里发IDE的人强了很多了。如果有能力，还可以读读VSC的文档：[Documentation for Visual Studio Code](https://link.zhihu.com/?target=https%3A//code.visualstudio.com/docs)，并不复杂，体验一下英语的实际应用也不错哦。
...
如果还不知道怎么修改，可以自己百度或者b站搜“环境变量”看视频。大多不是C的但是区别不大，小心**别错删**。
...
[谭九鼎](https://www.zhihu.com/people/tan-jiu-ding)

一只普通的计算机爱好者

作者：https://www.zhihu.com/people/tan-jiu-ding

原答案：https://www.zhihu.com/question/30315894/answer/154979413